  ...
  void saveUpdateEmployee(String name, String address, String docID, int addEditFlag){
    ...
    if(addEditFlag==1){
      ...
      collectionReference.add({'name':name,'address':address}).whenComplete((){
        ...
        clearEditing();
        Get.back();
        CustomSnackBar.showSnackBar(context: Get.context, title: "Employee Added", message: "Employee Added "
            "Successfilly", backColor: Colors.red);
      }).catchError((error){
        CustomFullScreenDialog.cancelDialog();
        CustomSnackBar.showSnackBar(context: Get.context, title: "Error", message: "Something went wrong",
